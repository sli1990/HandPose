...
def augmentation(input): # add possibility for leaving the image unchanged
	...
	# Random translate
	randTrans = tf.constant([0.0, 20.0])
	randTransZ = 0.0
...
def parse_function(example_proto):
	...
	return image_reshaped, tf.cast(joint_dec, tf.float32)
...
def run_augmentation():
	...
	next_images, next_joints = iterator.get_next()

	# Build Graph
	im_out, joint_out = augnet(next_images, next_joints)
