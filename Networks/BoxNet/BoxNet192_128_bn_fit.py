...
def boundboxnet(images):
	"""BoundBoxNet model to be used for inference
  	...
  	# Placeholder to indicate if training or inference is taking place (needed for batch normalization)
  	...
  	with tf.name_scope('convolution1'):
		...
		activation = tf.nn.conv2d(padded_input, weights, strides=[1, 1, 1, 1], padding='VALID')
		batch_normalized = tf.layers.batch_normalization(activation, training=training_var)
		hidden1 = tf.nn.relu(batch_normalized)
  	...
  	with tf.name_scope('convolution2'):
		...
		activation = tf.nn.conv2d(hidden1_pool, weights, strides=[1, 1, 1, 1], padding='SAME')
		batch_normalized = tf.layers.batch_normalization(activation, training=training_var)
		hidden2 = tf.nn.relu(batch_normalized)
  	...
  	with tf.name_scope('convolution3'):
		...
		activation = tf.nn.conv2d(hidden2_pool, weights, strides=[1, 1, 1, 1], padding='SAME')
		batch_normalized = tf.layers.batch_normalization(activation, training=training_var)
		hidden3 = tf.nn.relu(batch_normalized)
  	...
  	with tf.name_scope('fc1'):
  		weights = tf.Variable(tf.random_uniform([16*16*12,1024], minval=-math.sqrt(6/(16*16*12+1024)), maxval=math.sqrt(6/(16*16*12+1024))), name='weights')
		activation = tf.matmul(hidden3_flat, weights)
		batch_normalized = tf.layers.batch_normalization(activation, training=training_var)
		hidden4 = tf.nn.relu(batch_normalized)
  	...
  	with tf.name_scope('fc2'):
		...
		activation = tf.matmul(hidden4_drop, weights)
		batch_normalized = tf.layers.batch_normalization(activation, training=training_var)
		hidden5 = tf.nn.relu(batch_normalized)
  	...
  	return box_offset, keep_prob, training_var
...
def run_training():
	...
	box_offset_est, keep_prob, training_var = boundboxnet(next_images)
    ...
    # Build the summary Tensor based on the TF collection of Summaries.
   	...
   	# Define extra update operations for batch normalization
    ...
    # Create a saver for writing training checkpoints (the 10 most recent are kept).
   	...
   	eval_train_steps_per_epoch = 469 #int(math.floor(FLAGS.train_size/FLAGS.batch_size))
    ...
    # Run the Op to initialize the variables.
   	...
   	eval_frequency = 469 #FLAGS.train_size/FLAGS.batch_size
  	...
  	print('START TRAINING')
   	...
   	for steps in xrange(FLAGS.max_steps):	#TODO: change to training epochs
		...
		if steps % log_frequency == log_frequency_comp:
			...
			summary, _, _ = sess.run([merged, train_op, extra_update_ops], feed_dict={handle: training_handle, keep_prob: 1.0, training_var: True}, options=run_options, run_metadata=run_metadata)
			train_writer.add_run_metadata(run_metadata,'step%d' % steps)
			train_writer.add_summary(summary,steps)
			train_writer.flush()
		else:
			sess.run([train_op, extra_update_ops], feed_dict={handle: training_handle, keep_prob: 1.0, training_var: True})
		...
		if steps % eval_frequency == eval_frequency_comp:
			...
			for _ in range(eval_train_steps_per_epoch):
				eval_loss += sess.run(eval_op, feed_dict={handle: eval_train_handle, keep_prob: 1.0, training_var: False})
