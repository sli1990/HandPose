...
def run_boxnet():
	...
	training_filenames, validation_filenames = dataset_filenames(FLAGS.data_dir, 'CHALLENGE')
	handle = tf.placeholder(tf.string, shape=[])
	next_images, next_joints, next_offsets, next_scales, training_iterator, eval_train_iterator, validation_iterator = create_datasets_boxnet(training_filenames, validation_filenames, handle, FLAGS.batch_size, 4, 10000)

	# Build a Graph that computes predictions from the BoundBoxNet inference model
	box_offset_est = BoxNet(next_images, next_joints, next_offsets, next_scales)

	# Add the operations for loss calculation to the graph
	eval_op = loss_boxnet(box_offset_est, next_offsets)
	...
	var_list = tf.contrib.framework.filter_variables(boxnet_vars, exclude_patterns=['Adam'])
	saver_red = tf.train.Saver(var_list, max_to_keep=10)
	...
	if FLAGS.run_mode=='inference':
		...
		for steps in xrange(FLAGS.start_step ,FLAGS.max_steps):
			...
			if steps % eval_frequency == eval_frequency_comp:
				...
				checkpoint_file_red = os.path.join(FLAGS.log_dir, 'model_red.ckpt')
				saver_red.save(sess, checkpoint_file_red, global_step=steps)
...
if __name__ == '__main__':
  ...
  parser.add_argument(
	  '--log_dir',
	  type=str,
	  default='/home/dhri-dz/Documents/HandPose/T1NEW/Log',
