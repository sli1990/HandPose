...
def boundboxnet(images):
	"""BoundBoxNet model to be used for inference
  	...
  	with tf.name_scope('flatten'):
  		hidden3_flat = tf.reshape(hidden3, [-1, 24*24*12])

  	# Fully Connected Layer 1 with Xavier initialization
  	with tf.name_scope('fc1'):
  		weights = tf.Variable(tf.random_uniform([24*24*12,1024], minval=-math.sqrt(6/(24*24*12+1024)), maxval=math.sqrt(6/(24*24*12+1024))), name='weights')
...
def parse_function(example_proto):
	...
	return image_reshaped, tf.cast(offset_dec, tf.float32)
