...
def parse_function(example_proto):
	...
	return image_reshaped, joint_dec, offset_dec, handScale_dec
...
def parse_function_augment(example_proto):
	...
	# Data Augmentation
	image_reshaped, joint_dec, offset_dec, handScale_dec = tf.py_func(augmentation_cv,[image_reshaped, joint_dec, offset_dec, handScale_dec],[tf.float32, tf.float32, tf.float32, tf.float32])
	image_reshaped = tf.reshape(image_reshaped,[176,176,1])
...
def create_datasets_boxnet(training_filenames, validation_filenames, handle, batch_size, thread_count, buffer_count):
	...
	training_dataset = training_dataset.map(parse_function, num_threads=thread_count)
	...
	return next_images, next_joints, next_offsets, next_scales, training_iterator, eval_train_iterator, validation_iterator