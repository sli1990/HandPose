...
def run_scalenet():
	...
	training_filenames, validation_filenames = dataset_filenames(FLAGS.data_dir, 'TEST128C')
	handle = tf.placeholder(tf.string, shape=[])
	next_images, next_joints, next_scales, training_iterator, eval_train_iterator, validation_iterator = create_datasets_boxnet(training_filenames, validation_filenames, handle, FLAGS.batch_size, 8, 1000)

	# Build a Graph that computes predictions from the BoundBoxNet inference model
	hand_scale_est = ScaleNet(next_images, next_joints, next_scales)
	...
	train_op = train_scalenet(eval_scale_op, FLAGS.learning_rate)
