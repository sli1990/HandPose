...
def BoxScaleNet(images,joints,offsets,scales):
	"""BoxScaleNet model
  	Args:	
  	...
  	# Scope vairables for BoxNet
  	with tf.name_scope('boxnet'):
	  	...
	  	with tf.name_scope('flatten'):
	  		hidden3_flat = RESHAPE2D(hidden3, 22, 12)

	  	# Fully Connected Layer 1
	  	with tf.name_scope('fc1'):
	  		hidden4 = FULLY_CONNECTED(hidden3_flat, 22*22*12, 1024, 0, False, 0.0)
	  	...
	  	with tf.name_scope('linear'):
	  		box_offset = LINEAR_OUTPUT(hidden5, 1024, 3, 0)

	# Crop 128x128 image centered on MCP
	images_centered, joints_centered = tf.map_fn(process_box2scale, [images, joints, box_offset], dtype=(tf.float32, tf.float32))

	# Stop gradient from flowing through BoxNet
	images_centered = tf.stop_gradient(images_centered)
	...
	# Scope vairables for ScaleNet
  	...
  	return box_offset, hand_scale


def loss_boxnet(bboffset, label):
...
def train_boxscalenet(loss, learning_rate):
