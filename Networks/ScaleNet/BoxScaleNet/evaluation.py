...
def evaluation_function_box(sess, eval_op, initializable_iterator, handle, data_handle, data_size, steps_per_epoch, mode, reg_param):
...
def inference_fun_box(sess, eval_op, eval_train_iterator, validation_iterator, handle, eval_train_handle, validation_handle, train_size, validation_size, eval_train_steps_per_epoch, validation_steps_per_epoch):

	# EVALUATION RUN ON TRAINING SET
	evaluation_function_box(sess, eval_op, eval_train_iterator, handle, eval_train_handle, train_size, eval_train_steps_per_epoch, 'TRAINING', 3)

	# EVALUATION RUN ON VALIDATION SET
	evaluation_function_box(sess, eval_op, validation_iterator, handle, validation_handle, validation_size, validation_steps_per_epoch, 'VALIDATION', 3)
