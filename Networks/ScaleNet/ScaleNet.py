...
def scalenet(images):
	"""ScaleNet model to be used for inference
  	...
  	with tf.name_scope('linear'):
  		weights = tf.Variable(tf.random_uniform([1024,1], minval=-math.sqrt(6/(1024+1)), maxval=math.sqrt(6/(1024+1))), name='weights')
...
def run_inference():
	"""Do inference on the hand image data with ScaleNet."""
	...
	validation_handle = sess.run(validation_iterator.string_handle())
   	...
   	# Restore the model
   	checkpoint_file = os.path.join(FLAGS.log_dir, 'model.ckpt-14999')
