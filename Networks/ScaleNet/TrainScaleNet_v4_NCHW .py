...
def scalenet(images):
	"""ScaleNet model to be used for inference
  	...
  	# Transpose padded input tensor from NHWC to NCHW format
  	with tf.name_scope('transp'):
  		transp_input = tf.transpose(padded_input, perm=(0, 3, 1, 2))
  	...
  	with tf.name_scope('convolution1'):
		...
		hidden1 = tf.nn.relu(tf.nn.conv2d(transp_input, weights, strides=[1, 1, 1, 1], padding='VALID', data_format='NCHW') + biases)
	#hidden1 = lrelu(tf.nn.conv2d(padded_input, weights, strides=[1, 1, 1, 1], padding='VALID') + biases)

  	# Pooling Layer 1
  	with tf.name_scope('pool1'):
  		hidden1_pool = tf.nn.max_pool(hidden1, ksize=[1, 1, 4, 4], strides=[1, 1, 4, 4], padding='SAME', data_format='NCHW')
  	...
  	with tf.name_scope('convolution2'):
  		...
  		hidden2 = tf.nn.relu(tf.nn.conv2d(hidden1_pool, weights, strides=[1, 1, 1, 1], padding='SAME', data_format='NCHW') + biases)
  		#hidden2 = lrelu(tf.nn.conv2d(hidden1_pool, weights, strides=[1, 1, 1, 1], padding='SAME') + biases)

  	# Pooling Layer 2
  	with tf.name_scope('pool2'):
  		hidden2_pool = tf.nn.max_pool(hidden2, ksize=[1, 1, 2, 2], strides=[1, 1, 2, 2], padding='SAME', data_format='NCHW')
  	...
  	with tf.name_scope('convolution3'):
  		...
  		hidden3 = tf.nn.relu(tf.nn.conv2d(hidden2_pool, weights, strides=[1, 1, 1, 1], padding='SAME', data_format='NCHW') + biases)
  	...
  	with tf.name_scope('linear'):
  		weights = tf.Variable(tf.random_uniform([1024,1], minval=-math.sqrt(6/(1024+3)), maxval=math.sqrt(6/(1024+3))), name='weights')
  		biases = tf.Variable(tf.zeros([1]), name='biases')
  		scale = tf.matmul(hidden5, weights) + biases
  		#scale = tf.matmul(hidden5, weights) + biases 
  	...
  	print('START TRAINING')
   	...
   	for steps in xrange(FLAGS.max_steps):	#TODO: change to training epochs
		...
		if steps % eval_frequency == eval_frequency_comp:
			...
			# Print ǘalidation data evaluation results
			duration = time.time() - start_time
