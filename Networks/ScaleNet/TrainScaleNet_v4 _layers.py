...
def scalenet(images):
	"""ScaleNet model to be used for inference
  	...
  	with tf.name_scope('convolution1'):
  		hidden1 = tf.layers.conv2d(padded_input, 12, [5, 5], activation=tf.nn.relu, kernel_initializer=tf.contrib.layers.xavier_initializer)

  	# Pooling Layer 1
  	with tf.name_scope('pool1'):
  		hidden1_pool = tf.layers.max_pooling2d(hidden1, 4, 4, padding='same')

  	# Convolutional Layer 2
  	with tf.name_scope('convolution2'):
  		hidden2 = tf.layers.conv2d(hidden1_pool, 12, [5, 5], padding = 'same', activation=tf.nn.relu, kernel_initializer=tf.contrib.layers.xavier_initializer)

  	# Pooling Layer 2
  	with tf.name_scope('pool2'):
  		hidden2_pool = tf.layers.max_pooling2d(hidden2, 2, 2, padding='same')

  	# Convolutional Layer 3
  	with tf.name_scope('convolution3'):
  		hidden3 = tf.layers.conv2d(hidden2_pool, 12, [3, 3], padding = 'same', activation=tf.nn.relu, kernel_initializer=tf.contrib.layers.xavier_initializer)
  	...
  	with tf.name_scope('fc1'):
  		hidden4 = tf.layers.dense(hidden3_flat, 1024, activation=tf.nn.relu, kernel_initializer=tf.contrib.layers.xavier_initializer)

  	# Fully Connected Layer 2
  	with tf.name_scope('fc2'):
  		hidden5 = tf.layers.dense(hidden4, 1024, activation=tf.nn.relu, kernel_initializer=tf.contrib.layers.xavier_initializer)

  	# Output Layer
  	with tf.name_scope('linear'):
  		scale = tf.layers.dense(hidden5, 1, kernel_initializer=tf.contrib.layers.xavier_initializer)
