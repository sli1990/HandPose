...
def visualization(image):
	...
	#im_flip = tf.transpose(tf.squeeze(image))
	#im_flip = tf.reshape(im_flip,[192,192,1])
	
	return im_flip#tf.transpose(image)#im_flip
...
def augmentation(input): # add possibility for leaving the image unchanged
	...
	offsets_x, offsets_y, offsets_z = rotate((0,0), offsets_x, offsets_y, offsets_z, -randAngle)

	# Change the ground truth
	joints_x = joints_x + randTrans[0]*(0.135/64)
	joints_y = joints_y + randTrans[1]*(0.135/64)
	joints_z = joints_z + randTransZ
	joints = tf.stack([joints_x, joints_y, joints_z], axis=1)
	joints = tf.reshape(joints, [63])
	offsets_x = offsets_x + randTrans[0]*(0.135/64)
	offsets_y = offsets_y + randTrans[1]*(0.135/64)
	...
	return image, joints, offsets, scales#, randTrans[0], randTrans[1]
...
def process_box2scale(input):
	...
	offset_x_px = offset_x*(64/0.135)
	offset_y_px = offset_y*(64/0.135)
	...
	image = tf.image.crop_to_bounding_box(image, crop_start_y, crop_start_x, 128, 128)	# x and y flipped due to tensor indexing
...
def augnet(images, joints, offsets, scales):
	...
	images_aug, joints_aug, offsets_aug, scales_aug = tf.map_fn(augmentation, [images, joints, offsets, scales], dtype=(tf.float32, tf.float32, tf.float32, tf.float32))
...
def parse_function(example_proto):
	...
	return image_reshaped, tf.cast(joint_dec, tf.float32), tf.cast(offset_dec, tf.float32), tf.cast(handScale_dec, tf.float32)
...
def run_augmentation():
	...
	next_images, next_joints, next_offsets, next_scales = iterator.get_next()

	# Build Graph
	im_out, joint_out, offset_out, scale_out = augnet(next_images, next_joints, next_offsets, next_scales)
