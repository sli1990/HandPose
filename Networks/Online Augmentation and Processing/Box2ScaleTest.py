...
def parse_function(example_proto):
	...
	return image_reshaped, tf.cast(joint_dec, tf.float32), tf.cast(offset_dec, tf.float32)
...
def run_augmentation():
	...
	next_images, next_joints, next_offsets = iterator.get_next()

	# Build Graph
	im_out, joint_out, off_out = b2sNet(next_images, next_joints, next_offsets)
