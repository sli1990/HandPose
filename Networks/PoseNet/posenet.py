...
from layers import *
from KinLayerMAT import KinLayer, PCA_Layer, ScaleLayer
...
def PoseNet(images,joints,scales):
	"""PoseNet model
  	...
  	Returns:
    	mcpBase: 6D MCP (middle) base position
    	fingerVectors: 3D vectors from MCP base (middle) to other finger bases (MCP) and wrist base
    	jointAngles: angles of the finger joints
    	boneShapes: PCA parameters for bone lengths
    	baseShapes: pCA parameters for base shapes
  	"""
  	...
  	with tf.name_scope('posenet'):
	  	...
	  	# Regress 6D MCP (middle) base
	  	with tf.name_scope('mcpBase'):	
		  	...
		  	with tf.name_scope('linear'):
		  		mcpBase = LINEAR_OUTPUT(hidden5_a, 1024, 6, 0)

		# Regress finger base vectors (3D)
	  	with tf.name_scope('fingerVectors'):	

		  	# Fully Connected Layer 1
		  	with tf.name_scope('fc1'):
		  		hidden4_b = FULLY_CONNECTED(hidden3_flat, 16*16*12, 1024, 0, False, 0.0)

		  	# Fully Connected Layer 2
		  	with tf.name_scope('fc2'):
		  		hidden5_b = FULLY_CONNECTED(hidden4_b, 1024, 1024, 0, False, 0.0)

		  	# Linear Output Layer
		  	with tf.name_scope('linear'):
		  		fingerVectors = LINEAR_OUTPUT(hidden5_b, 1024, 15, 0)

	  	# Regress joint angles
	  	with tf.name_scope('jointAngles'):	

		  	# Fully Connected Layer 1
		  	with tf.name_scope('fc1'):
		  		hidden4_c = FULLY_CONNECTED(hidden3_flat, 16*16*12, 1024, 0, False, 0.0)

		  	# Fully Connected Layer 2
		  	with tf.name_scope('fc2'):
		  		hidden5_c = FULLY_CONNECTED(hidden4_c, 1024, 1024, 0, False, 0.0)

		  	# Linear Output Layer
		  	with tf.name_scope('linear'):
		  		jointAngles = LINEAR_OUTPUT(hidden5_c, 1024, 25, 0)

		# Regress bone length shape PCA parameters
	  	with tf.name_scope('boneShapes'):	

		  	# Fully Connected Layer 1
		  	with tf.name_scope('fc1'):
		  		hidden4_d = FULLY_CONNECTED(hidden3_flat, 16*16*12, 1024, 0, False, 0.0)

		  	# Fully Connected Layer 2
		  	with tf.name_scope('fc2'):
		  		hidden5_d = FULLY_CONNECTED(hidden4_d, 1024, 1024, 0, False, 0.0)

		  	# Linear Output Layer
		  	with tf.name_scope('linear'):
		  		boneShapes = LINEAR_OUTPUT(hidden5_d, 1024, 9, 0)

		# Regress directly 3D hand pose
	  	with tf.name_scope('poseDirect'):	

		  	# Fully Connected Layer 1
		  	with tf.name_scope('fc1'):
		  		hidden4_e = FULLY_CONNECTED(hidden3_flat, 16*16*12, 1024, 0, False, 0.0)

		  	# Fully Connected Layer 2
		  	with tf.name_scope('fc2'):
		  		hidden5_e = FULLY_CONNECTED(hidden4_e, 1024, 1024, 0, False, 0.0)

		  	# Linear Output Layer
		  	with tf.name_scope('linear'):
		  		poseDirect = LINEAR_OUTPUT(hidden5_e, 1024, 63, 0)

		# PCA backprojection
		with tf.name_scope('pca'):
			boneLengths = tf.map_fn(PCA_Layer, boneShapes, dtype=tf.float32, parallel_iterations=32)

		# Forward kinematics to arrive at 3D hand pose
		with tf.name_scope('kin'):
			pose3D = tf.map_fn(KinLayer, [mcpBase,fingerVectors,jointAngles,boneLengths], dtype=tf.float32, parallel_iterations=32)
  	...
  	return pose3D, mcpBase, fingerVectors, jointAngles, boneLengths, poseDirect 


def loss_posenet(pose3D, mcpBase ,fingerVectors, jointAngles, boneLengths, poseDirect, poseGT):
	"""Calculates the combined loss of the PoseNet.
	Args:
		pose3D: 3D pose estimated by PoseNet
		mcpBase: 6D MCP (middle) pose and orientation
		fingerVectors: regressed vectors from MCP base (middle) to other finger MCPs and wrist base
		jointAngles: regressed joint angles
		boneLengths: regressed bone lengths
		poseDirect: directly regressed 3D hand pose (without kinematic layer)
	...
	# Physical constraint loss on MCP base
	mcp_constr_lower = tf.Variable([-0.405, -0.405, -0.405, -4*math.pi, -4*math.pi, -4*math.pi]) # x,y,z,angles
	mcp_constr_upper = tf.Variable([ 0.405,  0.405,  0.405,  4*math.pi,  4*math.pi,  4*math.pi]) # x,y,z,angles

	mcpLossU = 2*tf.nn.l2_loss(tf.maximum((mcpBase-mcp_constr_upper),0.0))	# 2 needed?
	mcpLossL = 2*tf.nn.l2_loss(tf.minimum((mcpBase-mcp_constr_lower),0.0))	# 2 needed?
	mcpLoss = mcpLossU + mcpLossL

	# Physical constraint loss on bone lengths !!! CHANGE TO REASONABLE VALUES, CURRENTLY SIMPLY MIN/MAX CALCULATED FROM DATASET
	bone_constr_lower = tf.Variable([0.0117929, 0.0620296, 0.0586767, 0.0536472, 0.0544855,	# mcp to finger bases
									 0.0436496, 0.0295909, 0.0235260, 						# thumb (base to tip)
									 0.0369533, 0.0189618, 0.0156657,						# index (base to tip)
									 0.0421931, 0.0237336, 0.0174064, 						# middle (base to tip)
									 0.0380680, 0.0198928, 0.0182768, 						# ring (base to tip)
									 0.0286841, 0.0169650, 0.0156657])						# little (base to tip)

	bone_constr_upper = tf.Variable([0.0152104, 0.0802157, 0.0738560, 0.0700193, 0.0728969, # mcp to finger bases
									 0.0672793, 0.0468447, 0.0308885,						# thumb (base to tip)
									 0.0535961, 0.0386033, 0.0236303,						# index (base to tip)
									 0.0569878, 0.0457618, 0.0246148,						# middle (base to tip)
									 0.0548450, 0.0433070, 0.0239793,						# ring (base to tip)
									 0.0467911, 0.0354617, 0.0226457])						# little (base to tip)
   
   	boneLossU = 2*tf.nn.l2_loss(tf.maximum((boneLengths-bone_constr_upper),0.0))	# 2 needed?
   	boneLossL = 2*tf.nn.l2_loss(tf.minimum((boneLengths-bone_constr_lower),0.0))	# 2 needed?
	boneLoss = boneLossU + boneLossL
   
	# Physical constraint loss on joint angles
	angle_constr_lower = tf.Variable([-(55/180)*math.pi, -(55/180)*math.pi, -(50/180)*math.pi, -(15/180)*math.pi, -(35/180)*math.pi,	# thumb (base to tip)
                             		  -(20/180)*math.pi, -(50/180)*math.pi, -(25/180)*math.pi, -(15/180)*math.pi, -(25/180)*math.pi,	# index (base to tip)
                             		  -(20/180)*math.pi, -(50/180)*math.pi, -(25/180)*math.pi, -(15/180)*math.pi, -(25/180)*math.pi,	# middle (base to tip)
                             		  -(20/180)*math.pi, -(50/180)*math.pi, -(25/180)*math.pi, -(15/180)*math.pi, -(25/180)*math.pi,	# ring (base to tip)
                             		  -(20/180)*math.pi, -(50/180)*math.pi, -(25/180)*math.pi, -(15/180)*math.pi, -(25/180)*math.pi])	# little (base to tip)

	angle_constr_upper = tf.Variable([(90/180)*math.pi, (90/180)*math.pi, (50/180)*math.pi,  (90/180)*math.pi,  (90/180)*math.pi,	# thumb (base to tip)
                             		  (20/180)*math.pi, (50/180)*math.pi, (100/180)*math.pi, (100/180)*math.pi, (100/180)*math.pi,	# index (base to tip)
                             		  (20/180)*math.pi, (50/180)*math.pi, (100/180)*math.pi, (100/180)*math.pi, (100/180)*math.pi,	# middle (base to tip)
                             		  (20/180)*math.pi, (50/180)*math.pi, (100/180)*math.pi, (100/180)*math.pi, (100/180)*math.pi,	# ring (base to tip)
                             		  (20/180)*math.pi, (50/180)*math.pi, (100/180)*math.pi, (100/180)*math.pi, (100/180)*math.pi])	# little (base to tip)

	angleLossU = 2*tf.nn.l2_loss(tf.maximum((jointAngles-angle_constr_upper),0.0))	# 2 needed?
	angleLossL = 2*tf.nn.l2_loss(tf.minimum((jointAngles-angle_constr_lower),0.0))	# 2 needed?
	angleLoss = anglesLossU + angleLossL

	# PHYSICAL CONSTRAINT LOSS ON LENGTH OF FINGER VECTORS ???

	# Direct pose loss (for guidance of hand parameter estimation CNN)
	poseLossDirect = tf.nn.l2_loss(poseDirect-poseGT)
