...
def run_posenet():
	...
	training_filenames, validation_filenames = dataset_filenames(FLAGS.data_dir, 'TEST128CS')
	...
	pose3D_est = PoseNet(next_images, next_joints, next_scales)

	# Add the operations for loss calculation to the graph
	eval_op = loss_boxnet(pose3D_est, next_joints)
