...
def evaluation_function(sess, eval_op, initializable_iterator, handle, data_handle, data_size, steps_per_epoch, mode):
	...
	if mode=='TRAINING':
		print('TRAIN DATA EVALUATION: Num examples: %d  Avg loss per joint component: %.8f (%.2f mm) Time for evaluation: %.2f sec' % (data_size, eval_loss_avg, eval_loss_avg_mm, duration))
	elif mode=='VALIDATION':
		print('VALIDATION DATA EVALUATION: Num examples: %d  Avg loss per joint component: %.8f (%.2f mm) Time for evaluation: %.2f sec' % (data_size, eval_loss_avg, eval_loss_avg_mm, duration))
