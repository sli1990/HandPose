...
def dataset_filenames(data_dir,mode):
	...
	if mode=='CHALLENGE':
		...
		# Training and validation data locations
		training_filenames = ["Hands2017Train128S_1"]
							  
	  	validation_filenames = ["Hands2017Train128S_2"]
...
def parse_function(example_proto):
	...
	# IN CASE 176x176 DATASET IS USED: SIMULATE PERFECT BOXNET AND SCALENET BY USING GROUND TRUTH
	#image_reshaped, joint_dec = process_box2scale(image_reshaped,joint_dec,offset_dec)
	#image_reshaped = process_scale2pose(image_reshaped,handScale_dec)
...
def parse_function_augment(example_proto):
	...
	# Data Augmentation
	image_reshaped, joint_dec = tf.py_func(augmentation_cv,[image_reshaped, joint_dec, handScale_dec],[tf.float32, tf.float32])
	...
	# image_reshaped, joint_dec = augmentation(image_reshaped, joint_dec)
