...
def PoseNet(images,joints,scales):
	"""PoseNet model
  	...
  	Returns:
    	wristBase: 6D wrist base position
    	fingerVectors: 3D vectors from wrist base to finger bases (MCP)
  	...
  	with tf.name_scope('posenet'):
	  	...
	  	# Regress 6D wrist base
	  	with tf.name_scope('wristBase'):	
		  	...
		  	with tf.name_scope('linear'):
		  		wristBase = LINEAR_OUTPUT(hidden5_a, 1024, 6, 0)
		...
		with tf.name_scope('kin'):
			pose3D = tf.map_fn(KinLayer, [wristBase,fingerVectors,jointAngles,boneLengths], dtype=tf.float32, parallel_iterations=32)
  	...
  	return pose3D, wristBase, fingerVectors, jointAngles, boneLengths, poseDirect 


def loss_posenet(pose3D, wristBase ,fingerVectors, jointAngles, boneLengths, poseDirect, poseGT):
	"""Calculates the combined loss of the PoseNet.
	Args:
		pose3D: 3D pose estimated by PoseNet
		wristBase: 6D wrist pose and orientation
		fingerVectors: regressed vectors from wrist base to finger MCPs
	...
	# Physical constraint loss on wrist base
	wrist_constr_lower = tf.Variable([-0.405, -0.405, -0.405, -4*math.pi, -4*math.pi, -4*math.pi]) # x,y,z,angles
	wrist_constr_upper = tf.Variable([ 0.405,  0.405,  0.405,  4*math.pi,  4*math.pi,  4*math.pi]) # x,y,z,angles

	wristLossU = 2*tf.nn.l2_loss(tf.maximum((wristBase-wrist_constr_upper),0.0))	# 2 needed?
	wristLossL = 2*tf.nn.l2_loss(tf.minimum((wristBase-wrist_constr_lower),0.0))	# 2 needed?
	wristLoss = wristLossU + wristLossL

	# Physical constraint loss on bone lengths !!! CHANGE TO REASONABLE VALUES, CURRENTLY SIMPLY MIN/MAX CALCULATED FROM DATASET
	bone_constr_lower = tf.Variable([0.0117929, 0.0620296, 0.0586767, 0.0536472, 0.0544855,	# wrist to finger bases
	...
	bone_constr_upper = tf.Variable([0.0152104, 0.0802157, 0.0738560, 0.0700193, 0.0728969, # wrist to finger bases
