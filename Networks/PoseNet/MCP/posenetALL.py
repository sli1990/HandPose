...
def PoseNet(images,joints,scales):
	"""PoseNet model
  	...
  	Returns:
    	...
    	fingerAngles: angles of the finger joints
  	...
  	with tf.name_scope('posenet'):
	  	...
	  	with tf.name_scope('linear'):
	  		handParam = LINEAR_OUTPUT(hidden5_a, 1024, 55, 0)
  	
		# PCA backprojection
		with tf.name_scope('pca'):
			boneLengths = tf.map_fn(PCA_Layer, handParam[46:55], dtype=tf.float32, parallel_iterations=32)

		# Forward kinematics to arrive at 3D hand pose
		with tf.name_scope('kin'):
			pose3D = tf.map_fn(KinLayer, [handParam[0:6],fingerVectors[6:21],jointAngles[21:46],boneLengths], dtype=tf.float32, parallel_iterations=32)
  	...
  	return pose3D, mcpBase, fingerVectors, jointAngles, boneLengths


def loss_posenet(pose3D, mcpBase ,fingerVectors, jointAngles, boneLengths, poseGT):
