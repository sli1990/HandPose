...
% visualize that in camera aligned coordinates u,v shift corresponds to x,y
% shift
[umcp,vmcp,dmcp] = xyz2uvd(xyz(1,4),xyz(2,4),xyz(3,4));
[offset2x,offset2y] = size(d);
figure('Name','Correspondence u,v and x,y')
imshow(d)
hold on
title('Correspondence u,v and x,y')
plot(umcp-cropStart(1),vmcp-cropStart(2),'rx','MarkerSize',10)
plot(xyzDemean(1,4)+offset2x/2,xyzDemean(2,4)+offset2y/2,'bx','MarkerSize',5)
hold off
...
x_plot = round(boundingBoxOffset(1)+83.5);
y_plot = round(boundingBoxOffset(2)+83.5);
plot(x_plot,y_plot,'g*','Markersize',10)
plot(83.5,83.5,'r*','Markersize',10)
drawArrow([x_plot 83.5],[y_plot 83.5],'linewidth',1,'color','r')
hold off
...
% helper variables
mid_shift = [83.5-x_plot 83.5-y_plot];
crop_offset = (166-128)/2;
...
u_crop_ref_end = round(166 - crop_offset - mid_shift(1)); 
v_crop_ref_start = round(crop_offset+1 - mid_shift(2));
v_crop_ref_end = round(166 - crop_offset - mid_shift(2));
...
% visualization    
hand3d_v = ones(166,1)*(1:166);
hand3d_u = (1:166)' * ones(1,166);
hand3d_d = d;
