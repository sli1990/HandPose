...
for frameIndex=1:60000
    ...
    % resize normalized image to 176x176 with nearest neighbor
    % interpolation and save size before re-sizing
    d = imresize(squeeze(d),[176,176], 'nearest'); % squeeze removes singleton dimensions
    ...
    %% Center on MCP
    % crop centered on MCP: 128x128
    boundingBoxOffset(1) = max(min(boundingBoxOffset(1),50.625),-50.625);
    boundingBoxOffset(2) = max(min(boundingBoxOffset(2),50.625),-50.625);
    boundingBoxOffset(3) = max(min(boundingBoxOffset(3),100),-100);
    
    offset_x_px = round(boundingBoxOffset(1)*(64/135));
	offset_y_px = round(boundingBoxOffset(2)*(64/135));
    offset_z = boundingBoxOffset(2)/235;
    
    d_crop = d(24+offset_y_px+1:152+offset_y_px,24+offset_x_px+1:152+offset_x_px);
    
    % re-center depth
    dval = 135/235;
    d_crop = d_crop-offset_z;
    d_crop(d_crop<-dval) = dval;
    d_crop(d_crop>dval) = dval;
    d_crop = d_crop/dval;

    % change joint ground truth
    xyzDemean = xyzDemean - boundingBoxOffset';
    
    %% Scale-normalize the images
    % Limit the scale value
    bonelengthsScales(frameIndex) = max(min(bonelengthsScales(frameIndex),2),0.5);
    
    % Pad the image with ones
    d_scale = ones(256);
    d_scale(65:192,65:192) = d_crop;
    
    % Crop image and resize
	start_sc = round(128-64*bonelengthsScales(frameIndex));
    end_sc = round(128+64*bonelengthsScales(frameIndex));
	d_scale = d_scale(start_sc+1:end_sc,start_sc+1:end_sc);
    d_scale = imresize(d_scale,[128,128],'nearest');
    
    % Resize the depth values
    d_scale(d_scale<0.999) = max(min((d_scale(d_scale<0.999)/bonelengthsScales(frameIndex)),1),-1);

    %% Write HDF5 file
    % depth image
    depthImages(:,:,1,batchIndex) = d_scale;
    ...
    if (mod(frameIndex,h5fileSize)==0 || frameIndex==957032)
         h5filename = ['Hands2017Train128S_',num2str(ceil(frameIndex/h5fileSize)),'.h5'];
         h5filename
         delete(h5filename);
         h5create(h5filename,'/depth',[128,128,1,h5fileSize],'Datatype','single');
         h5write(h5filename,'/depth', depthImages);
         h5create(h5filename,'/joint',[63,h5fileSize],'Datatype','single');
         h5write(h5filename,'/joint',joints);
         h5create(h5filename,'/handScale',[1,h5fileSize],'Datatype','single');
