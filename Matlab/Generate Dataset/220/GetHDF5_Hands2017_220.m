...
h5fileSize = 30000;
depthImages=zeros(220,220,1,h5fileSize);
...
sizeBeforeResize = zeros(2,size(xyzs,1));
...
for frameIndex=1:957032
    ...
    % resize normalized image to 220x220 with nearest neighbor
    % interpolation and save size before re-sizing
    sizeBeforeResize(:,frameIndex) = size(d);
    d = imresize(squeeze(d),[220,220], 'nearest'); % squeeze removes singleton dimensions
    ...
    if (mod(frameIndex,h5fileSize)==0 || frameIndex==957032)
         h5filename = ['Hands2017Train220_',num2str(ceil(frameIndex/h5fileSize)),'.h5'];
         h5filename
         delete(h5filename);
         h5create(h5filename,'/depth',[220,220,1,h5fileSize],'Datatype','single');
         ...
         h5create(h5filename,'/fingerScale',[20,h5fileSize],'Datatype','single');
         h5write(h5filename,'/fingerScale',fingerScales);
         h5create(h5filename,'/center3ds',[3,h5fileSize],'Datatype','single');
         h5write(h5filename,'/center3ds',center3ds);
         h5create(h5filename,'/center3dsOrig',[3,h5fileSize],'Datatype','single');
         h5write(h5filename,'/center3dsOrig',center3dsOrig);
         h5create(h5filename,'/offset',[3,h5fileSize],'Datatype','single');
