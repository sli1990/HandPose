...
h5fileSize = 30000;
depthImages=zeros(166,166,1,h5fileSize);
...
cropScale=zeros(2,h5fileSize); 

%% Calculate the components of the HDF5 file 

for frameIndex=1:h5fileSize
    ...
    filename = ['image_D',num2str(frameIndex,'%08d'),'.png'];
    ...
    %% Padding
    % Pad 600 zeros around every image border
    ...
    % resize normalized image to 166x166 with nearest neighbor
    % interpolation and save size before re-sizing
    sizeBeforeResize = size(d);
    cropScaling = [sizeBeforeResize(2)/166 sizeBeforeResize(1)/166];
    d = imresize(squeeze(d),[166,166], 'nearest'); % squeeze removes singleton dimensions
    
    %% 3x3 median filtering
    d = medfilt2(d,[3,3]);
    
    %% Determine bounding box shift
    % x and y scaled to coincide with pixel values in cropped 166x166
    % image --> multiply with cropScaling for changing the ground truth
    boundingBoxOffset = [xyzDemean(1,4)/cropScaling(1) xyzDemean(2,4)/cropScaling(2) xyzDemean(3,4)];
    ...
    %% Write HDF5 file
    % joint ground truth centered and rotated divided by 1000 (?)
    ...
    % for re-centering
    centerOffset(:,batchIndex) = boundingBoxOffset;
    cropScale(:,batchIndex) = cropScaling;
    
    if (mod(frameIndex,h5fileSize)==0 || frameIndex==957032)
         h5filename = ['Hands2017Train_',num2str(ceil(frameIndex/h5fileSize)),'.h5'];   
         h5filename
         delete(h5filename);
         ...
         h5create(h5filename,'/handScales',[1,h5fileSize],'Datatype','double');
         h5write(h5filename,'/handScales',scales);
         h5create(h5filename,'/fingerScales',[20,h5fileSize],'Datatype','double');
         h5write(h5filename,'/fingerScales',fingerScales);
         ...
         h5create(h5filename,'/centerOffset',[3,h5fileSize],'Datatype','double');
         h5write(h5filename,'/centerOffset',centerOffset);
         h5create(h5filename,'/cropScale',[2,h5fileSize],'Datatype','double');
         h5write(h5filename,'/cropScale',AugScales);
